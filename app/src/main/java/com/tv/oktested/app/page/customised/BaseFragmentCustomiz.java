...
public class BaseFragmentCustomiz extends BrandedFragmentCustomi {

    /**
     * The start state for all
     */
    final StateMachineCustomiz.State STATE_START = new StateMachineCustomiz.State("START", true, false);

    /**
     * Initial State for ENTRNACE transition.
     */
    final StateMachineCustomiz.State STATE_ENTRANCE_INIT = new StateMachineCustomiz.State("ENTRANCE_INIT");

    /**
     * prepareEntranceTransition is just called, but view not ready yet. We can enable the
     * busy spinner.
     */
    final StateMachineCustomiz.State STATE_ENTRANCE_ON_PREPARED = new StateMachineCustomiz.State("ENTRANCE_ON_PREPARED", true, false) {
        @Override
        public void run() {
     ...
     * in this StateMachineCustomiz.State, the process is very different in subclass, e.g. BrowseFragment hide header
     * views and hide main fragment view in two steps.
     */
    final StateMachineCustomiz.State STATE_ENTRANCE_ON_PREPARED_ON_CREATEVIEW = new StateMachineCustomiz.State(
     ...
     */
    final StateMachineCustomiz.State STATE_ENTRANCE_PERFORM = new StateMachineCustomiz.State("STATE_ENTRANCE_PERFORM") {
        @Override
        public void run() {
     ...
     */
    final StateMachineCustomiz.State STATE_ENTRANCE_ON_ENDED = new StateMachineCustomiz.State("ENTRANCE_ON_ENDED") {
        @Override
        public void run() {
     ...
     */
    final StateMachineCustomiz.State STATE_ENTRANCE_COMPLETE = new StateMachineCustomiz.State("ENTRANCE_COMPLETE", true, false);

    /**
     * Event fragment.onCreate()
     */
    final StateMachineCustomiz.Event EVT_ON_CREATE = new StateMachineCustomiz.Event("onCreate");

    /**
     * StateMachineCustomiz.Event fragment.onViewCreated()
     */
    final StateMachineCustomiz.Event EVT_ON_CREATEVIEW = new StateMachineCustomiz.Event("onCreateView");

    /**
     * Event for {@link #prepareEntranceTransition()} is called.
     */
    final StateMachineCustomiz.Event EVT_PREPARE_ENTRANCE = new StateMachineCustomiz.Event("prepareEntranceTransition");

    /**
     * Event for {@link #startEntranceTransition()} is called.
     */
    final StateMachineCustomiz.Event EVT_START_ENTRANCE = new StateMachineCustomiz.Event("startEntranceTransition");

    /**
     * Event for entrance transition is ended through Transition listener.
     */
    final StateMachineCustomiz.Event EVT_ENTRANCE_END = new StateMachineCustomiz.Event("onEntranceTransitionEnd");
    ...
    final Condition COND_TRANSITION_NOT_SUPPORTED = new Condition("EntranceTransitionNotSupport") {
        ...
        public boolean canProceed() {
            return !TransitionHelperCustomized.systemSupportsEntranceTransitions();
        }
    };

    final StateMachineCustomiz mStateMachine = new StateMachineCustomiz();

    Object mEntranceTransition;
    final ProgressBarManager mProgressBarManager = new ProgressBarManager();

    @SuppressLint("ValidFragment")
    BaseFragmentCustomiz() {
    }
...
//                COND_TRANSITION_NOT_SUPPORTED);
    ...
    void onExecuteEntranceTransition() {
        ...
        view.getViewTreeObserver().addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
            ...
            public boolean onPreDraw() {
               ...
               /* if (FragmentUtil.getContext(BaseFragmentCustomiz.this) == null || getView() == null) {
    ...
    void internalCreateEntranceTransition() {
     ...
     /*   TransitionHelperCustomized.addTransitionListener(mEntranceTransition, new TransitionHelperCustomized() {
            @Override
