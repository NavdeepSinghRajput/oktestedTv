...
import com.tv.oktested.app.page.presenterView.ShowsCardPresenterView;
 ...
 */
public class ShowCardsPresenter extends PresenterSelector {
    private final HashMap<ShowsListModel, Presenter> presenters = new HashMap<ShowsListModel, Presenter>();

    @Override
    public Presenter getPresenter(Object item) {
        if (!(item instanceof ShowsListModel)) throw new RuntimeException(
                String.format("The PresenterSelector only supports data items of type '%s'",
                        ShowsListModel.class.getName()));
        ShowsListModel card = (ShowsListModel) item;
        Presenter presenter = presenters.get(card);
        if (presenter == null) {
            presenter = new ShowsCardPresenterView();
            return presenter;
        } else {
            return null;
        }
